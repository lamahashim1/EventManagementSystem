...
def event_manager(request):
    if request.user.is_authenticated and request.user.is_admin:
        eventlist = Event.objects.values('event_id', 'event_name', 'event_status').exclude(event_status="Delete")
        event_heads = {}
...
def event_participation_list(request, event_id):
    if request.user.is_authenticated and request.user.is_admin:
        ...
        event_name = Event.objects.filter(event_id = event_id).exclude(event_status="Delete").values('event_name')[0]['event_name']
