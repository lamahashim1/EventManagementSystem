    ...
    if request.method == 'POST':
        ...
        if usercheck and len(usercheck) == 1:
            ...
            if user:
                ...
                events = Event.objects.exclude(event_status="Delete").values('event_name', 'fees')
...
def event_detail(request):
    ...
    if query is not None:
        ...
        event_list = Event.objects.exclude(event_status="Delete").filter(event_name__icontains=query)
    else:    
        # events = Event.objects.values('event_name','event_detail', 'rules', 'event_logo', 'fees', 'event_status')
        event_list = Event.objects.exclude(event_status="Delete")
