...
def coordinator_confirm_participation(request):
    ...
    if request.user.is_authenticated and ec and ec[0].is_coordinator:
        if Coordinator.objects.filter(reg_no = request.user.reg_no, isActive = True):
            ...
            participant_list = Participants.objects.values('reg_no', 'reg_no__fname', 'reg_no__lname', 'reg_no__email', 'reg_no__contect_no', 'reg_no__user_stream__stream_name', 'remark', 'total_payment', 'remaining_payment', 'paid_payment', 'is_paid')
            events = {}
            is_confirmed = {}
